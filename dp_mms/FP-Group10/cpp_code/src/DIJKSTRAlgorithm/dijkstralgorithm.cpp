...
//	std::unique_ptr<fp::MinHeap> heap = std::make_unique<fp::MinHeap>();
	...
	//! Set all cost as -1 for unexplored regions except starting node.
	for(int x=0; x<maze_ptr->WIDTH; ++x){
		for(int y=0; y<maze_ptr->HEIGHT; ++y){
			if( x==wheel_robot->get_x() and y==wheel_robot->get_y() ) cost2come_[x][y] = 0;
			else cost2come_[x][y] = std::numeric_limits<double>::max() ;
		}
	}
	
	//! Push initial nodes to heap
//	heap->push(getKey(wheel_robot->get_x(), wheel_robot->get_y()), 0.0);
	push(std::make_pair(getKey(wheel_robot->get_x(), wheel_robot->get_y()), 0.0));
	...
	while( not empty() ){
		...
		curr_x = getX(top().first);
		curr_y = getY(top().first);
...
//		if(cost2come_[curr_x][curr_y] < dist) continue;
		...
		for(int i=0; i<4; ++i){			
			if( maze_ptr->isCellValid(curr_x + xarr_[i], curr_y + yarr_[i]) ){			
				std::cerr << curr_x << "," << curr_y << "    " << curr_x + xarr_[i] << "," <<  curr_y + yarr_[i] << std::endl;
				if( not maze_ptr->isWall(curr_x, curr_y, dir_[i]) ){
					...
					if( maze_ptr->isVisited(curr_x + xarr_[i], curr_y + yarr_[i]) ){
						if( cost2come_[curr_x+xarr_[i]][curr_y+yarr_[i]] > cost2come_[curr_x][curr_y] + 1.0 ){
							...
							push(std::make_pair(getKey(curr_x + xarr_[i], curr_y + yarr_[i]), cost2come_[curr_x+xarr_[i]][curr_y+yarr_[i]]));
	...
	std::cerr << "path is " << std::endl;
	std::cerr << curr_x << "," << curr_y << std::endl;
